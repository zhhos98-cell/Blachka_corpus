...
This directory is the persistent working state for the Blaschka Object Network. It exists so that a new research session can recover the current census, open problems, evidential rules, and next actions directly from GitHub.

## Canonical working files

### Data

- [`data/current_holders.csv`](data/current_holders.csv) — holder-first working census: one row per current/recent holder plus explicit audit tombstones or recheck candidates.
- [`data/census.csv`](data/census.csv) — broader candidate and historical-lead layer, including successor routes that do not yet meet current-holder proof.
- [`data/survey_baselines.csv`](data/survey_baselines.csv) — aggregate historical survey chronology.
- [`data/country_baseline_2016.csv`](data/country_baseline_2016.csv) — country-level reconstruction of Corning's 64 current collections in 2016, compared with the project's secure holder distribution.

### Working files

- [`working/`](working/) — bounded active investigations, ingest priorities, and temporary structured research outputs that have not yet become canonical tables.
- [`RESEARCH_LOG.md`](RESEARCH_LOG.md) — dated research decisions, discoveries, corrections, and handoff notes.
- [`logs/`](logs/) — detailed session-specific research notes when a run is too granular for the main log.
- [`bibliography/`](bibliography/) — working bibliographies tied to specific research questions.
...
`data/current_holders.csv` contains **66 audit rows**:
...
The 2016 Corning country chart has been converted into a gap audit. Its dark-blue `Current` bars sum to the published total of 64. The most useful deficits against the present project table are **Slovenia 2 missing**, **Germany 3 missing against secure rows (2 if Görlitz is confirmed)**, **Australia 1 missing**, **Switzerland 1 missing**, and **United Kingdom 1 missing**. These deficits are higher-priority discovery targets than a fresh country-by-country global sweep. Surpluses in Austria, Italy, New Zealand, and the United States are comparison leads only; they do not prove post-2016 additions until individual nodes are matched.
...
The 2017 definition includes museums, schools, and individuals. Therefore “68” does not mean 68 museums. The project uses a delta audit: reconstruct the 2017 surviving baseline as closely as possible, then identify current surviving nodes that cannot be matched to that baseline.
...
1. Update `data/current_holders.csv` and/or `data/census.csv` for every node whose status changed.
2. Update aggregate baselines in `data/` when a historical survey is reconstructed more precisely.
...
4. Put bounded active investigations in `working/`; promote them into canonical tables only when their status warrants it.
5. Keep uncertainty explicit. Do not silently convert historical counts into current counts, loans into ownership, OCR numbers into prices or object counts, or successor institutions into item-level provenance.
6. In a new session, read this file, `data/current_holders.csv`, `data/country_baseline_2016.csv`, `data/census.csv`, and the latest log entry first.

This structure is intended to make the repository itself sufficient for research handoff without mixing temporary investigations with canonical data.
