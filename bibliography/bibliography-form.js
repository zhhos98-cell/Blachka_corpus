(() => {
  /*
    Bibliography v1 is already flattened into index.html and bibliography-data.json.
    The historical bibliography-form.js previously replayed old pass records at runtime,
    which duplicated entries because the frozen HTML uses data-source-pass while the old
    replay guard checked data-bib-pass. Keep this filename as a compatibility shim, but do
    not add records here.
  */
  ...
  const fold = value => clean(value)
    ...
    .replace(/[\u0300-\u036f]/g, '')
    .toLowerCase();
  const normDOI = value => clean(value)
  ...
  const doiOf = node => {
    for (const link of node.querySelectorAll('.bib-links a')) {
      try {
        ...
        if (url.hostname.toLowerCase() === 'doi.org') return normDOI(url.href);
      } catch {}
    }
  ...
  [...list.querySelectorAll('.bib-entry')].forEach(node => {
    ...
    const citationKey = `${year}|${fold(citation)}`;

    const duplicate = (doi && seenDOI.has(doi)) || seenCitation.has(citationKey);
    if (duplicate) {
      node.remove();
      return;
    }
